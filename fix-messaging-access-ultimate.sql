-- ULTIMATE MESSAGING ACCESS FIX
-- This handles all existing policies and fixes the messaging system completely
...
DROP POLICY IF EXISTS "Users can view event participants" ON message_participants;
...
DROP POLICY IF EXISTS "Users can send event messages" ON messages;
...
DROP POLICY IF EXISTS "Users can mark messages as read" ON message_read_status;
DROP POLICY IF EXISTS "Users can update read status" ON message_read_status;
...
DROP POLICY IF EXISTS "Users can view private chat messages" ON private_chat_messages;
DROP POLICY IF EXISTS "Users can send private chat messages" ON private_chat_messages;
...
DROP POLICY IF EXISTS "Users can view private chat participants" ON private_chat_participants;
DROP POLICY IF EXISTS "Users can join their private chats" ON private_chat_participants;
DROP POLICY IF EXISTS "Users can view their private chat participants" ON private_chat_participants;
DROP POLICY IF EXISTS "Users can join their private chats" ON private_chat_participants;

-- 2. CREATE NEW, UNIQUE POLICIES FOR MESSAGE_PARTICIPANTS
-- Users can view participants for events they're part of
CREATE POLICY "mp_view_participants" ON message_participants
  FOR SELECT USING (
    user_id = auth.uid() OR
    event_id IN (
      SELECT mp2.event_id 
...
CREATE POLICY "mp_join_conversations" ON message_participants
  FOR INSERT WITH CHECK (user_id = auth.uid());

-- Users can update their own participation
CREATE POLICY "mp_update_participation" ON message_participants
  FOR UPDATE USING (user_id = auth.uid());

-- 3. CREATE NEW, UNIQUE POLICIES FOR MESSAGES
-- Users can view messages for events they participate in
CREATE POLICY "msg_view_messages" ON messages
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "msg_send_messages" ON messages
...
-- 4. CREATE NEW, UNIQUE POLICIES FOR MESSAGE_READ_STATUS
-- Users can view read status for messages they have access to
CREATE POLICY "mrs_view_read_status" ON message_read_status
  FOR SELECT USING (
    user_id = auth.uid() OR
    message_id IN (
...
CREATE POLICY "mrs_mark_read" ON message_read_status
  FOR INSERT WITH CHECK (user_id = auth.uid());

-- Users can update their read status
CREATE POLICY "mrs_update_read_status" ON message_read_status
  FOR UPDATE USING (user_id = auth.uid());

-- 5. CREATE NEW, UNIQUE POLICIES FOR PRIVATE CHATS
-- Users can view private chat messages they participate in
CREATE POLICY "pcm_view_messages" ON private_chat_messages
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY "pcm_send_messages" ON private_chat_messages
  FOR INSERT WITH CHECK (
    sender_id = auth.uid() AND
    EXISTS (
      SELECT 1 FROM private_chat_participants pcp 
...
CREATE POLICY "pcp_view_participants" ON private_chat_participants
  FOR SELECT USING (
    user_id = auth.uid() OR
    chat_id IN (
      SELECT pcp2.chat_id 
...
CREATE POLICY "pcp_join_chats" ON private_chat_participants
  FOR INSERT WITH CHECK (user_id = auth.uid());
...
-- 9. VERIFY THE ULTIMATE FIX
DO $$
BEGIN
  RAISE NOTICE 'Ultimate messaging access fix applied successfully';
...
-- 10. FINAL STATUS
SELECT 'Ultimate messaging access fix applied - messages should now be visible' as status;
