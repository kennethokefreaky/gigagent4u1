-- SIMPLE, FOCUSED MESSAGING SYSTEM FIX
-- Implements exactly what you want: private messages (promoter + talent only) and group messages (promoter + candidates)

-- 1. DROP ALL EXISTING POLICIES TO START FRESH
DROP POLICY IF EXISTS "mp_view_participants" ON message_participants;
DROP POLICY IF EXISTS "mp_join_conversations" ON message_participants;
DROP POLICY IF EXISTS "mp_update_participation" ON message_participants;
DROP POLICY IF EXISTS "msg_view_messages" ON messages;
DROP POLICY IF EXISTS "msg_send_messages" ON messages;
DROP POLICY IF EXISTS "mrs_view_read_status" ON message_read_status;
DROP POLICY IF EXISTS "mrs_mark_read" ON message_read_status;
DROP POLICY IF EXISTS "mrs_update_read_status" ON message_read_status;
DROP POLICY IF EXISTS "pcm_view_messages" ON private_chat_messages;
DROP POLICY IF EXISTS "pcm_send_messages" ON private_chat_messages;
DROP POLICY IF EXISTS "pcp_view_participants" ON private_chat_participants;
DROP POLICY IF EXISTS "pcp_join_chats" ON private_chat_participants;

-- 2. CREATE SIMPLE PRIVATE CHAT POLICIES
-- Only the promoter and talent can see private messages
CREATE POLICY "private_chat_view" ON private_chat_messages
  FOR SELECT USING (
    EXISTS (
...
-- Only the promoter and talent can send private messages
CREATE POLICY "private_chat_send" ON private_chat_messages
...
-- Only the promoter and talent can see private chat participants
...
-- Only the promoter and talent can join private chats
CREATE POLICY "private_participants_join" ON private_chat_participants
  FOR INSERT WITH CHECK (user_id = auth.uid());

-- 3. CREATE SIMPLE GROUP CHAT POLICIES
-- Only event participants (promoter + candidates) can see group messages
CREATE POLICY "group_chat_view" ON messages
  FOR SELECT USING (
    EXISTS (
...
-- Only event participants (promoter + candidates) can send group messages
CREATE POLICY "group_chat_send" ON messages
  FOR INSERT WITH CHECK (
    sender_id = auth.uid() AND
    EXISTS (
      SELECT 1 FROM message_participants mp 
...
-- Only event participants can see group chat participants
...
-- Only event participants can join group chats
CREATE POLICY "group_participants_join" ON message_participants
  FOR INSERT WITH CHECK (user_id = auth.uid());

-- Only event participants can update their participation
CREATE POLICY "group_participants_update" ON message_participants
  FOR UPDATE USING (user_id = auth.uid());

-- 4. CREATE SIMPLE READ STATUS POLICIES
-- Only event participants can see read status
...
-- Only event participants can mark messages as read
CREATE POLICY "read_status_mark" ON message_read_status
  FOR INSERT WITH CHECK (user_id = auth.uid());

-- Only event participants can update read status
...
-- Function to create notifications for private messages (promoter + talent only)
...
-- Function to create notifications for group messages (promoter + candidates only)
...
-- 9. VERIFY THE SIMPLE, FOCUSED FIX
DO $$
BEGIN
  RAISE NOTICE 'Simple, focused messaging system implemented successfully';
  RAISE NOTICE 'Private messages: Only promoter + talent can see and send';
  RAISE NOTICE 'Group messages: Only promoter + candidates can see and send';
  RAISE NOTICE 'Notifications will be created automatically for new messages';
END $$;

-- 10. FINAL STATUS
SELECT 'Simple, focused messaging system implemented - exactly as requested' as status;
