-- FIX PROMOTER PARTICIPATION IN GROUP CHATS
-- This ensures promoters are automatically added as participants
-- Run this in your Supabase SQL editor

-- 1. ADD KEITH CAROLS TO ALL EXISTING GROUP CHATS
-- First, let's see what group chats exist
SELECT 'EXISTING GROUP CHATS' as section;
SELECT DISTINCT event_id, p.title as event_title, p.user_id as promoter_id
...
SELECT DISTINCT 
    ...
    NOW() as joined_at
FROM posts p
WHERE p.user_id = (SELECT id FROM profiles WHERE email LIKE '%keith%' OR full_name LIKE '%keith%' LIMIT 1)
AND p.id NOT IN (
